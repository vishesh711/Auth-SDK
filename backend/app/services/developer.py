...
from app.utils import hash_password, verify_password, validate_password_strength, create_access_token
...
class DeveloperAuthService:
    ...
    async def login(
        ...
        if not developer or not verify_password(credentials.password, developer.password_hash):
            raise HTTPException(
