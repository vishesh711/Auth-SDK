...
class APIKeyService:
    ...
    async def create_api_key(
        self, db: AsyncSession, app_id: str, developer_id: UUID, key_data: APIKeyCreate
    ) -> tuple[APIKey, str]:
        """
        ...
        api_key = APIKey(app_id=app_id, label=key_data.label, key_hash=key_hash)
        db.add(api_key)
        await db.commit()
    ...
    async def list_api_keys(
        self, db: AsyncSession, app_id: str, developer_id: UUID
    ) -> list[APIKey]:
        """
    ...
    async def revoke_api_key(
        self, db: AsyncSession, app_id: str, key_id: UUID, developer_id: UUID
    ) -> bool:
        """
        Revoke an API key
        ...
        # Find API key
        stmt = select(APIKey).where(APIKey.id == key_id, APIKey.app_id == app_id)
        result = await db.execute(stmt)
