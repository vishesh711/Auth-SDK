...
@router.post("/developers/signup", response_model=DeveloperResponse, status_code=status.HTTP_201_CREATED)
async def developer_signup(
    developer_data: DeveloperSignup,
    db: AsyncSession = Depends(get_db)
) -> DeveloperResponse:
    """Register a new developer"""
    developer = await developer_auth_service.signup(db=db, developer_data=developer_data)
    return developer


@router.post("/developers/login", response_model=DeveloperTokenResponse)
async def developer_login(
    credentials: DeveloperLogin,
    db: AsyncSession = Depends(get_db)
):
    """Login developer"""
    developer, access_token = await developer_auth_service.login(db=db, credentials=credentials)
...
@router.post("/applications", response_model=ApplicationWithSecret, status_code=status.HTTP_201_CREATED)
...
@router.post("/applications/{app_id}/api-keys", response_model=APIKeyWithPlaintext, status_code=status.HTTP_201_CREATED)
...
@router.delete("/applications/{app_id}/api-keys/{key_id}", status_code=status.HTTP_200_OK)
